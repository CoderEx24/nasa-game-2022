    ...
    while run:
        ...
        if space_count4 < 16 and stat_show:
            ...
            if space_count3 > 4:
                space_count3 = 0
            if space_count3 >= 0:
                if STORY_LIST3[1 + space_count4] == 1 and k > (WIDTH / 2 - 150):
                    text.write(STORY_LIST3[0 + space_count4], WIDTH/2 - 300, HEIGHT/6 + 80)
            if space_count3 >= 2:
                if STORY_LIST3[3 + space_count4] == 1:
                    text.write(STORY_LIST3[2 + space_count4], WIDTH/2 - 300, HEIGHT/6 + 120)
            if space_count3 == 4:
                if STORY_LIST3[3 + space_count4] == 1:
                    text.write(STORY_LIST3[4 + space_count4], WIDTH/2 - 300, HEIGHT/6 + 160)
        ...
        if win:
            win_label = main_font.render("Congratulations!!", 1, (255, 255, 255))
            WIN.blit(win_label, (WIDTH / 2 - win_label.get_width() / 2, 100))
            win_label = main_font.render("You HAVE PASSED ALL THE DUST!! AND REACHED MARS", 1, (255, 255, 255))
            WIN.blit(win_label, (WIDTH / 2 - win_label.get_width() / 2, 200))
            win_label = main_font.render("UNFORTUNATELY", 1, (255, 255, 255))
            WIN.blit(win_label, (WIDTH / 2 - win_label.get_width() / 2, 300))
            win_label = main_font.render("MARS WAS NOT THE RIGHT PLANET FOR OUR MISSION", 1, (255, 255, 255))
