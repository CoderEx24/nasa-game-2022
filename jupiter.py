    ...
    while run:
        ...
        if space_count4 < 11 and stat_show:
            ...
            if space_count3 > 4:
                space_count3 = 0
            if space_count3 >= 0:
                if STORY_LIST5[1 + space_count4] == 1 and k > (WIDTH / 2 - 150):
                    text.write(STORY_LIST5[0 + space_count4], WIDTH/2 - 300, HEIGHT/6 + 80)
            if space_count3 >= 2:
                if STORY_LIST5[3 + space_count4] == 1:
                    text.write(STORY_LIST5[2 + space_count4], WIDTH/2 - 300, HEIGHT/6 + 120)
            if space_count3 == 4:
                if STORY_LIST5[3 + space_count4] == 1:
                    text.write(STORY_LIST5[4 + space_count4], WIDTH/2 - 300, HEIGHT/6 + 160)
            ...
            if len(enemies) == 0:
                ...
                for j in range(wave_length):
                    enemy = Enemy(random.randrange(50, WIDTH - 100), random.randrange(-2500, -100), 100, rock, 30, 30)
                    enemies.append(enemy)
                    enemy = Enemy(random.randrange(50, WIDTH - 100), random.randrange(-2500, -100), 100,
                                  pygame.transform.rotate(rock, 90), 40, 40)
                    enemies.append(enemy)
                    enemy = Enemy(random.randrange(50, WIDTH - 100), random.randrange(-2500, -100), 100,
                                  pygame.transform.rotate(rock, 180), 50, 50)
                    enemies.append(enemy)
        player.draw(WIN)
        ...
        if win:
            ...
            win_label = lost_font.render("You HAVE PASSED ALL THE ASTEROIDS!!", 1, (255, 255, 255))
